...
return $return_string.' ago';
	...
	function add_new_order($bill,$customername,$email,$center,$timings,$phone,$timenow,$address,$chef_note){
$order_status=0;
$d1=new dbconn();
$db=$d1->connect();
$mainstring=$this->return_order_string();
$timings=get_present_time();
$order_code=$_SESSION['product_chain'];
// STEP 1: UPDATE TO SYAYSTEM DATABASE ORDER DEPARTMENT 
$arr1=array();			
$orderid=$this->new_order_id();
$cmd="INSERT into my_orders(items,bill,customer,customer_email,order_id,center,timings,cust_phone,order_time,address,order_code,chef_note) values ('$mainstring','$bill','$customername','$email','$orderid','$center','$timings','$phone','$timenow','$address','$order_code','$chef_note')";
...
$link="monkhub.com/rest/index.php?trackme=".$orderid;
$link_name="Track Order";
$this->memberEmail($email,$subject,$email_admin,$link,$link_name);
// customer message ===================
//=========== order notifications ==============================
create_order_invoice($customername,$phone,$email,$address,$bill,$mainstring,$time,$orderid,$plan_id);
//add to admin notifications ===========================

//add to admin notifications ===========================
$notifications="Web online order ID :".$orderid;
$triggeredby=$email;
...
function return_delivery_boy_name($user_name){
	$arr1=array();				
	        						...
	        						$cmd="select name,phone from delivery_man where username='$user_name'";
