...
function return_all_icons($iconfor){
  	$d1=new dbconn();$arr1=array();
	               	$db=$d1->connect();
	               	$uploadedby=REST_KEY;
$cmd="select * from image_uploads where image_for='$iconfor' AND uploadedby='$uploadedby'";
$result=$db->query($cmd);
		...
		function operational_data(){ 
				     ...
				     $result=$db->query($cmd);
				      while($row = $result->fetch(PDO::FETCH_ASSOC))
				                 ...
				                 $this->rest_name=$arr1[0]['rest_name'];
				                 $this->address=$arr1[0]['address'];
				                 $this->minm_order=$arr1[0]['minmorder'];
			...
			function controller_data(){ 
				$rest_key=REST_KEY;
									...
									$d1=new dbconn();
										$db=$d1->connect();
                     $cmd="select * from web_controllers where rest_key='$rest_key'";
				     $result=$db->query($cmd);
				      while($row = $result->fetch(PDO::FETCH_ASSOC))
				                        {
				                   	array_push($arr1,$row);
