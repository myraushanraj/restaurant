...
return $return_string.' ago';
	...
	// retrive all delivery man
	function retrive_all_delivery_man(){
...
$cmd="INSERT into my_orders(items,bill,customer,customer_email,order_id,center,timings,cust_phone,order_time,address,source) values ('$mainstring','$bill','$customername','$email','$orderid','$center','$timings','$phone','$timenow','$address','store')";
$result=$db->query($cmd);
if($result)
$order_status=1;
...
$link="monkhub.com/rest/admin/";
$link_name="Acess admin";
...
//=========== order notifications ==============================
create_order_invoice($customername,$phone,$email,$address,$bill,$mainstring,$timings,$orderid,$plan_id,$domain);
...
<h3 style='text-align:center;'>VS Herbals</h3>
<table>
<tr><td><td><img src='http://monkhub.com/vikash/images/logt.png' width='140'></td></tr>
...
function all_data(){
	...
	$cmd="SELECT * from my_orders where order_id='$this->id'";
				                 ...
				                 // provide all details 
				                    $this->delivery_boy_name='Not updated';
				                     $this->delivery_boy_phone='Not updated';
                                    ...
                                    if(isset($arr1[0]['delivery_user']))
                                    list($this->delivery_boy_phone,$this->delivery_boy_name)=$this->return_delivery_boy_name($arr1[0]['delivery_user']);
                                    $this->invoice_link='';
                                    	if($this->source!='user')
{
$this->invoice_link='<a href="admin-scripts/invoices/IN'.$this->id.'.pdf" target="_blank">
									<img src="https://cdn4.iconfinder.com/data/icons/vectory-finance-2/40/invoice_check-512.png" width="45">
								</a>';

 }else{
$this->invoice_link='<a href="../callscripts/invoices/IN'.$this->id.'.pdf" target="_blank">
									<img src="https://cdn4.iconfinder.com/data/icons/vectory-finance-2/40/invoice_check-512.png" width="45">
								</a>';
...
function return_delivery_boy_name($user_name){
	$arr1=array();				
									$d1=new dbconn();
									$db=$d1->connect();

	        						$cmd="select name,phone from delivery_boy where delivery_user='$user_name'";
	        						$result=$db->query($cmd);
                                       while($row=$result->fetch()){ 
				                   	                   $phone=$row[0]['phone'];
				                   	                   $name=$row[0]['name'];
