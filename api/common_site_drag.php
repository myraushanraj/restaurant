...
$domain="https://www.taj-indiancuisine.com/menu/";
...
echo "tr is ".$search;
			foreach ($search->find('.portfolio2') as $searchtr){
					echo '<br>';
					echo 'td is '.$searchtr;

// save the datta in useful form 
			$item=$search->find('.menu-title');

			$pic_link=$search->find('td')[1];
			
			$description=$search->find('.menu_excerpt');

			$food_type=$search->find('td')[3];


echo "values are ".$item." and ".$description."<br>";
